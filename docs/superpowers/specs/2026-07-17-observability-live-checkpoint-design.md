...
`SqliteStore.snapshot_operational_event_stream(since, limit=2000)` uses one SQLite read transaction and one connection. Initialization guarantees a reserved opaque origin tombstone at sequence 0, before any real stream row. The snapshot reads the current maximum stream ledger row, exposes only that row's opaque `event_id` as `stream_after_id`, and selects the newest surviving `operational_events` whose ledger sequence is at or below the captured high-water and whose timestamp is within the requested window. It returns at most 2,000 items in chronological display order. Ledger rows remain valid markers after their event payload is pruned; a fresh database returns the origin ID with no items. The private integer sequence and origin tombstone never enter event payloads.
...
The browser requests the atomic seed first. Only the latest seed generation may merge state or open the stream. Every successful seed has a nonempty marker and opens exactly one EventSource using `after_id=stream_after_id`; a missing marker is treated as a seed error. The server resolves that durable ledger marker, so all rows inserted after the snapshot drain in pages of 100, including more than 100 rows and reconnects before the first delivered frame.
